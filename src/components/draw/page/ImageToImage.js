...
import {API_CONFIG, BASE_SIZES, IMAGE_STYLES, MULTIPLIERS} from "../../../constants";

const ImageToImage = ({
  imageStrength,
  setImageStrength,
  handleImageToImageUpload,
  imagePrompt,
  selectedCategory,
  selectedImageStyle,
  setImagePrompt,
  ...
  const handleOptimizePrompt = async (currentPrompt) => {
    ...
    try {
      const response = await fetch(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.optimizePrompt}`,{
        method: 'POST',
  ...
  return (
    <div className="min-h-full bg-gray-50 p-6">
      <div className="bg-white rounded-lg p-6">
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 cursor-pointer"
             onClick={() => document.querySelector('input[type="file"]').click()}>
          <div className="text-center">
            <p className="text-gray-600 mb-2">点击/拖拽上传图片</p>
            <p className="text-gray-400 text-sm">支持10M以内，JPG、PNG图片</p>
            <input
              type="file"
              accept="image/*"
              className="hidden"
              onChange={handleImageToImageUpload}
            />
          </div>
        </div>
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-600">风格强度</span>
            <span className="text-sm text-gray-400">{imageStrength}%</span>
          </div>
          <input
            type="range"
            min="0"
